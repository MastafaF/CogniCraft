...
def hybrid_search_opensearch(client, model, query, bm25_weight=0.5, semantic_weight=0.5):
    """Performs a hybrid search using OpenSearch."""
    ...
    for hit in bm25_results['hits']['hits']:
        combined_scores[hit['_id']] = combined_scores.get(hit['_id'], 0) + (hit['_score'] * bm25_weight)
    for hit in semantic_results['hits']['hits']:
        combined_scores[hit['_id']] = combined_scores.get(hit['_id'], 0) + (hit['_score'] * semantic_weight)
...
def initialize_faiss_bm25_services(documents, model):
    ...
    # --- BM25 Setup ---
    # BM25 needs tokenized text. We'll just split by space for this example.
...
def hybrid_search_faiss_bm25(query, model, bm25, faiss_index, documents, bm25_weight=0.5, semantic_weight=0.5):
    ...
    # RRF is a simple and effective way to combine ranks from different systems.
    # The formula is sum(1 / (k + rank)) for each document. k is a constant (e.g., 60)
    rrf_scores = {}
    
    # Process BM25 results
    bm25_ranked_indices = np.argsort(bm25_scores)[::-1] # Get indices of docs in descending score order
    for rank, doc_idx in enumerate(bm25_ranked_indices):
        rrf_scores[doc_idx] = rrf_scores.get(doc_idx, 0) + (1 / (60 + rank))

    # Process FAISS results
    # The 'indices' are the indices of the documents in the original list
    for rank, doc_idx in enumerate(indices[0]):
        if doc_idx != -1: # FAISS can return -1 for no result
            rrf_scores[doc_idx] = rrf_scores.get(doc_idx, 0) + (1 / (60 + rank))
...
if __name__ == "__main__":
    # You need to have OpenSearch running locally for the first part to work.
    # Docker: docker run -p 9200:9200 -p 9600:9600 -e "discovery.type=single-node" opensearchproject/opensearch:latest
    ...
    # Note: We don't need to re-initialize the sentence_model
    ...
    print("\n\n--- COMPARISON COMPLETE ---")
    print("Notice the architectural differences:")
    print("1. OpenSearch: A single, persistent service handles storage, keyword search, and vector search.")
    print("2. FAISS+BM25: In-memory, requires manual setup for both search types and a separate system for storing the original text.")

    print("Both approaches can be effective, but OpenSearch provides a more integrated solution for production use cases.")